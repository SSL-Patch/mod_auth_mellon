...
/* The default setting is to not use SSL when making outgoing requests
 */
static const int default_use_ssl = 0;
...
/* This function handles the MellonUseSSL configuration directive.
 ...
 *  NULL on success or an error string if the argument is wrong.
 */
static const char *am_set_use_ssl_slot(cmd_parms *cmd,
                                      void *struct_ptr,
                                      const char *arg)
{
    am_dir_cfg_rec *d = (am_dir_cfg_rec *)struct_ptr;
...
void *auth_mellon_dir_config(apr_pool_t *p, char *d)
{
    ...
    dir->sp_cert_path = NULL;    dir->idp_metadata = apr_array_make(p, 0, sizeof(am_metadata_t));
...
void *auth_mellon_dir_merge(apr_pool_t *p, void *base, void *add)
{
    ...
    new_cfg->use_ssl = (add_cfg->use_ssl != default_use_ssl ?
                        add_cfg->use_ssl :
                        base_cfg->use_ssl);
